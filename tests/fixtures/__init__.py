...
def changes_response_two():
    routes.update({('GET', '/v3/accounts/123-123-12345678-123/changes'): account_changes_response_two})
    yield
    routes.update({('GET', '/v3/accounts/123-123-12345678-123/changes'): account_changes_response})

@pytest.fixture
def all_trades_closed():
    routes.update({('GET', '/v3/accounts/123-123-12345678-123/openTrades'): no_open_trades_response})
    yield
    routes.update({('GET', '/v3/accounts/123-123-12345678-123/openTrades'): list_open_trades_response})
...
def all_trades_open_closed():
    ...
    routes.update({('GET', '/v3/accounts/123-123-12345678-123/openTrades'): list_trades_response()})
    yield
    routes.update({('GET', '/v3/accounts/123-123-12345678-123/openTrades'): list_open_trades_response})

...
def test_oanda_client_constructs_url(client):
    assert (
        client._hosts["REST"](path="/test").human_repr() == "http://127.0.0.1:8080/test"
    )
    assert (
        client._hosts["STREAM"](path="/test").human_repr()
...
async def test_client_initializes(client, server):
    ...
    try:
        ...
        assert client.default_parameters[AccountID] == AccountID("123-123-12345678-123")
    except:
        assert 0
    finally:
        await client.close()
...
async def test_client_raises_error_on_list_accounts_failure(
    client, server, error_status
):
...
@pytest.mark.asyncio
@pytest.mark.parametrize("error_status", error_status)
async def test_client_raises_error_on_get_account_details_failure(
    client, server, error_status
):
...
@pytest.mark.asyncio
@pytest.mark.parametrize("error_status", error_status)
async def test_client_raises_error_on_get_instruments_failure(
    client, server, error_status
):
    server_module.status = iter([400, 200, 200, 400])
    with pytest.raises(InitializationFailure):
        await client.initialize()
    assert client.initialized == False
    assert client.initializing == False
...
async def test_initialize_works_with_preset_account_id(client, server):
    client.account_id = "123-123-12345678-123"
    async with client as client:
        assert client.account_id == "123-123-12345678-123"
...
async def test_request_limiter_limits(client, server, event_loop):
    ...
    async with client as client:
        await asyncio.gather(
            *[client.list_orders() for _ in range(concurrent_requests)]
        )
...
async def test_client_handles_multiple_concurrent_initializations(
    ...
    try:
        await asyncio.gather(*[method(*data) for _ in range(5)], return_exceptions=True)
...
@pytest.mark.asyncio
async def test_close_all_trades_closes_all_trades(
    client, server, all_trades_open_closed
):
...
async def test_close_all_trades_error_second_list_trades_request(client, server):
    ...
    server_status = (
        i
        for i in chain(
            (200,), (200 for _ in range(len(close_all_trades_response))), (400,)
        )
    )
    async with client as client:
...
async def test_logger_captures_request_wait_time_when_in_debug(client, server, capsys):
    ...
    async with client as client:
        ...
        client.debug = True
        await asyncio.gather(*[client.account() for _ in range(2)])
...
# @pytest.mark.asyncio
# async def test_logger_warning_when_services_are_down(
#     client, server, capsys, services_down
# ):
#     logger = logging.getLogger("async_v20")
#     logger.addHandler(logging.StreamHandler())
#     logger.setLevel(logging.DEBUG)
...
#     for msg in (
#         "fxTrade Practice REST API Service is running without issue",
...
@pytest.mark.asyncio
async def test_account_calls_get_account_details_when_to_many_transactions_have_passed(
    client, server, caplog
):
    async with client as client:
...
@pytest.mark.asyncio
async def test_account_changes_logs_warning_when_too_many_transactions_have_passed(
    client, server, caplog
):
    async with client as client:
        client.default_parameters[LastTransactionID] += 986
    ...
    assert (
        "Too many transactions have passed to use the default "
        "`since_transaction_id` value." in caplog.text
    )


@pytest.mark.asyncio
async def test_changes_warning_too_many_transactions_have_passed_and_passed_value(
    client, server, caplog
):
    async with client as client:
        client.default_parameters[LastTransactionID] += 986
